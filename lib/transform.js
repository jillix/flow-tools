...
exports.state_transform = function (scope, state, args, data, next) {
    ...
    for (let key in args) {
        ...
        if (typeof args[key] === 'string') {
            data[key] = libob.path(args[key], state);
        } else {
            data[key] = libob.path(libob.path(args[key][1], data), libob.path(args[key][0], env), true);
        }
    }
